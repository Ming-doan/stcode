"""
Common tests — the elision contract, which is rule 1 in code.
...
`KeyError`. The fix is not "a better variable name" — it is that
...
def test_tracing_is_a_no_op_until_configured() -> None:
    """Off is the default, and off must cost nothing and break nothing: `span` is still
    a context manager and `set` still accepts attributes."""
