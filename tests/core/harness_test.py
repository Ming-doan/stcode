...
@pytest.fixture
def rt(context: HarnessContext) -> Runtime[HarnessContext]:
    return Runtime(context=context, approval_mode="full-auto")
...
def test_approval_is_requested_and_a_refusal_stops_the_call(
    context: HarnessContext, run: Any
) -> None:
    ...
    runtime = Runtime(context=context, approval_mode="suggest", on_approval=deny)
    ...
    assert not (context.cwd / "a.txt").exists()


def test_plan_mode_forbids_rather_than_asks(context: HarnessContext, run: Any) -> None:
    async def approve(_: ApprovalRequest) -> bool:
        raise AssertionError("plan mode must not even offer the prompt")

    runtime = Runtime(context=context, approval_mode="plan", on_approval=approve)
    result = run(write.invoke({"path": "a.txt", "content": "x"}, runtime))
...
def test_git_context_is_sampled_once_at_create_not_per_turn(tmp_path: Path, run: Any) -> None:
    ...
    the same step just bought — see docs/architecture/providers.md on caching."""
...
def test_bash_runs_in_a_given_subdirectory(tmp_path: Path, run: Any) -> None:
    ...
    the supported way to work somewhere else."""
    (tmp_path / "sub").mkdir()
...
def test_the_output_store_drops_the_oldest_spill() -> None:
    """Rule 1 keeps the full value reachable; nothing kept it from growing forever."""
    store = OutputStore(max_entries=3)
    for n in range(5):
        store[f"read_{n}"] = "x" * 10

    assert list(store) == ["read_2", "read_3", "read_4"]
    assert store.chars == 30


def test_reading_an_entry_keeps_it_alive() -> None:
    store = OutputStore(max_entries=2)
    store["a"], store["b"] = "1", "2"
    _ = store["a"]  # used, so "b" is now the oldest
    store["c"] = "3"

    assert set(store) == {"a", "c"}


def test_one_huge_value_evicts_the_rest_but_survives_itself() -> None:
    """The entry just written is the one the model was told about; dropping it would
    make the elision hint a lie."""
    store = OutputStore(max_entries=10, max_chars=100)
    store["small"] = "x" * 50
    store["huge"] = "y" * 5000

    assert list(store) == ["huge"]
